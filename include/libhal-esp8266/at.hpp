...
public:
  ...
  /**
   * @brief Construct a new at object
   *
   * Construction of the device will perform the following operations:
   *
   * 1. Reset device and wait for the "ready" response message
   * 2. Turn off echo back which helps reduce latency and wait for "ok" response
   *
   * If the expected responses do not come after the deadline, then the deadline
   * timeout controller will throw a hal::timed_out exception.
   *
   * @param p_serial - the serial port connected to the esp8266 device
